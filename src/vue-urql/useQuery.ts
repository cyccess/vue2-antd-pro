...
import { WatchStopHandle, Ref, ref, watchEffect, reactive, isRef } from '@vue/composition-api'
...
export type QueryPartialState<T = any, V = object> = Partial<OperationResult<T, V>> & { fetching?: boolean }
...
export type UseQueryResponse<T, V> = UseQueryState<T, V> & PromiseLike<UseQueryState<T, V>>

const watchOptions = {
  flush: 'pre' as const
}

export function useQuery<T = any, V = object>(args: UseQueryArgs<T, V>): UseQueryResponse<T, V> {
...
export function callUseQuery<T = any, V = object>(
  ...
  const isPaused: Ref<boolean> = isRef(_args.pause) ? _args.pause : ref(!!_args.pause)

  const request: Ref<GraphQLRequest<T, V>> = ref(
    createRequest<T, V>(args.query, unwrapPossibleProxy<V>(args.variables as V)) as any
  )
  ...
  stops.push(
    watchEffect(() => {
      const newRequest = createRequest<T, V>(args.query, unwrapPossibleProxy<V>(args.variables as V))
  ...
  stops.push(
    watchEffect(() => {
      ...
      source.value = !isPaused.value ? client.executeQuery<T, V>(request.value, _opts) : undefined
    }, watchOptions)
  )
  ...
  const state: UseQueryState<T, V> = {
    ...
    executeQuery(opts?: Partial<OperationContext>): UseQueryResponse<T, V> {
      ...
      source.value = client.executeQuery<T, V>(request.value, _opts)
