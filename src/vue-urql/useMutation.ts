...
export function callUseMutation<T = any, V = any>(
  ...
  return {
    ...
    operation,
    extensions,
    executeMutation(variables: V, context?: Partial<OperationContext>): Promise<OperationResult<T, V>> {
      fetching.value = true

      return pipe(
        client.executeMutation<T, V>(createRequest<T, V>(query, unwrapPossibleProxy<V>(variables)), context || {}),
        take(1),
        toPromise
