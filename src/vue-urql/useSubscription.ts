...
export type UseSubscriptionResponse<T = any, R = T, V = object> = UseSubscriptionState<T, R, V>
...
export function callUseSubscription<T = any, R = T, V = object>(
  ...
  stops.push(
    watchEffect(() => {
      ...
      source.value = !isPaused.value ? client.executeSubscription<T, V>(request.value, _opts) : undefined
    }, watchOptions)
  )
  ...
  const state: UseSubscriptionState<T, R, V> = {
    ...
    fetching,
    isPaused,
    executeSubscription(opts?: Partial<OperationContext>): UseSubscriptionState<T, R, V> {
      const _opts = {
        ...args.context,
        ...opts
      } as Partial<OperationContext>
      source.value = client.executeSubscription<T, V>(request.value, _opts)
